...
static const std::map<llm_tensor, llm_tensor_info> LLM_TENSOR_INFOS = {
    ...
    // NextN/MTP tensors live in the last nextn_predict_layers layer(s) (named blk.N.nextn.*),
    // so they are per-layer (REPEATING) tensors, NOT global output tensors. They were classified
    // as LAYER_OUTPUT while unused (always TENSOR_SKIP), which aborts at load once actually loaded
    // (loader: "input/output layer tensor ... used with a layer number"). REPEATING places them on
    // their layer's device and satisfies the layer-number requirement.
